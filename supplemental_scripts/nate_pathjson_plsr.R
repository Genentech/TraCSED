...
pathways_genes <- lapply(pathways_genes, function(genes) {
  genes <- which(rowData(sce)[['gene_id']] %in% genes)
  if (length(genes) == 0) {
    genes <- NULL
  }
...
pathway_scores <- sumCountsAcrossFeatures(sce, pathways_genes,
                                          exprs_values = "logcounts",
                                          average = TRUE)
pathway_scores <- as.data.frame(t(pathway_scores))
colnames(pathway_scores) <- paste0("Score.", colnames(pathway_scores))
...
dittoDimPlot(sce, "slingPseudotime_Global", reduction.use = "UMAP",
             size = point_size) + scale_color_viridis_c(direction = -1)
# trajectory
dittoDimPlot(sce, "resolution1.25", reduction.use = "UMAP", size = point_size,
             do.label = TRUE, legend.show = FALSE,
             add.trajectory.lineages = slingLineages(sce),
             trajectory.cluster.meta = "resolution1.25")
# Phases
...
dittoPlot(sce, "slingPseudotime_Avg2", group.by = "time_point",
          ...
          legend.show = FALSE, main = 'Pseudotime Distribution')

dittoPlot(sce, "slingPseudotime_Avg2", group.by = "phase",
          plots = c("ridgeplot"), ridgeplot.lineweight = 0.5,
          legend.show = FALSE, main = 'Pseudotime Distribution')

barcodes <- c('GFPBC_libB_83026', 'GFPBC_libB_40758', 'GFPBC_libB_20182', 'GFPBC_libB_90850', 'GFPBC_libB_37798', 'GFPBC_libB_92196', 'GFPBC_libB_10678', 'GFPBC_libB_94397', 'GFPBC_libB_31581')
for (b in barcodes){
  sce2 <- sce[, colData(sce)$gfp_bc == b]
  pl <- dittoPlot(sce2, "slingPseudotime_Avg2", group.by = "time_point",
            plots = c("ridgeplot"), ridgeplot.lineweight = 0.5,
            legend.show = FALSE, main = b)
  print(pl)
}
...
names(pdata) <- tnames
tjson <- toJSON(pdata)
write(tjson, "/gstore/project/hr_brca_heterogeneity/T47D_trace_Seq_v2/Nathan/tran_combo_BREAST.json")
